    ...
    const PaStreamParameters * output_parameters)
{
    static double standard_sample_rates[] =
    {
        ...
        44100.0f, 48000.0f, 88300.0f, 96000.0f, 192000.0f, -1.0f
    ...
    {
        error = Pa_IsFormatSupported(input_parameters, output_parameters,
...
static int paCallback(const void * input_buffer, void * output_buffer,
    uint64_t frames_per_buffer, const PaStreamCallbackTimeInfo * time_info,
...
static void exit_program(const int32_t type)
{
    destroy_all_waves();
    Pa_Terminate();
    exit(type);
...
int main(int argc, char** argv)
{
    ...
    while (1)
    {
        ...
        if (midi.note_status == 1)
            create_wave(SQUARE, get_note_frequency(midi.note),
                    get_note_amplitude(midi.velocity));
        else if (midi.note_status == 0)
            destroy_wave(get_note_frequency(midi.note));
